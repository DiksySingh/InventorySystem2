...
module.exports.addNewInstallationData = async (req, res) => {
  ...
  try {
    ...
    const systemItemsMapped = await SystemItemMap.find({ systemId }).session(
      session
    );
    let allItems = [];
    ...
    for (const itemMap of systemItemsMapped) {
      ...
      allItems.push({
        systemItemId: mainItemId.toString(),
    ...
    for (const item of allItems) {
      if (itemsMap.has(item.systemItemId)) {
        itemsMap.set(
          item.systemItemId,
          itemsMap.get(item.systemItemId) + item.quantity
        );
      } else {
    ...
    const finalItemsList = Array.from(itemsMap.entries()).map(
...
module.exports.filterServicePersonById = async (req, res) => {
  try {
    ...
    let employeeName = await ServicePerson.findById({ _id: id }).select(
      "-email -password -role -createdAt -isActive -refreshToken -__v -createdAt -updatedAt -createdBy -updatedBy"
    );
    if (!employeeName) {
