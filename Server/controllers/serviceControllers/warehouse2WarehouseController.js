...
module.exports.sendingDefectiveItems = async (req, res) => {
  try {
    ...
    const toWarehouseData = await Warehouse.findOne({
      warehouseName: toWarehouse,
    });
    const toWarehouseItemsData = await WarehouseItems.findOne({
      warehouse: toWarehouseData._id,
    });
    ...
    for (let item of items) {
      ...
      const toWarehouseItem = toWarehouseItemsData.items.find(
      ...
      const warehouseItems = warehouseItemsData.items.find(
        (i) => itemName === i.itemName
      );
      ...
      if (isDefective === true) {
        ...
        warehouseItems.defective =
          parseInt(warehouseItems.defective) - parseInt(quantity);
        ...
        warehouseItems.newStock =
          parseInt(warehouseItems.newStock) - parseInt(quantity);
        ...
        warehouseItems.quantity =
          parseInt(warehouseItems.quantity) - parseInt(quantity);
      }
    }

    await warehouseItemsData.save();

    const createDefectiveOrder = new WToW({
      fromWarehouse,
      toWarehouse,
      isDefective,
      items,
...
module.exports.inDefectiveItemsData = async (req, res) => {
  try {
    ...
    const incomingDefectiveData = await WToW.find({
      $and: [{ toWarehouse: warehouseData.warehouseName }, { status: false }],
    })
      .sort({ pickUpDate: -1 })
      .select("-__v");
...
module.exports.inDefectiveItemsOrderHistory = async (req, res) => {
  try {
    ...
    const warehouseData = await Warehouse.findOne({ _id: warehouseId });
    const incomingDefectiveData = await WToW.find({
      toWarehouse: warehouseData.warehouseName,
    })
      .sort({ pickUpDate: -1 })
      .select("-__v");
...
module.exports.outgoingDefectiveOrderData = async (req, res) => {
  try {
    ...
    const warehouseData = await Warehouse.findOne({ _id: warehouseId });
    const defectiveOrderData = await WToW.find({
...
module.exports.updateDefectiveOrderStatus = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const defectiveOrderData = await WToW.findById(defectiveOrderId).session(
        session
      );
      if (defectiveOrderData.status === true) {
      ...
      if (status === true && defectiveOrderData.isDefective) {
        for (let item of defectiveOrderData.items) {
          ...
          let warehouseItems = toWarehouseItemsData.items.find(
          ...
          warehouseItems.defective =
            parseInt(warehouseItems.defective || 0) + parseInt(quantity);
          console.log(
            `Updated defective stock for ${itemName}:`,
            warehouseItems.defective
          );
        }
      } else if (
        status === true &&
        !defectiveOrderData.isDefective &&
        defectiveOrderData.isNewStock
      ) {
        for (let item of defectiveOrderData.items) {
          const { itemName, quantity } = item;
          let warehouseItems = toWarehouseItemsData.items.find(
          ...
          if (!warehouseItems)
            throw new Error(`Item '${itemName}' not found in warehouse`);
          warehouseItems.newStock =
            parseInt(warehouseItems.newStock || 0) + parseInt(quantity);
          console.log(
            `Updated new stock for ${itemName}:`,
            warehouseItems.newStock
          );
        }
        toWarehouseItemsData.markModified("items");
        ...
        for (let item of defectiveOrderData.items) {
          ...
          if (!warehouseItems)
            throw new Error(`Item '${itemName}' not found in warehouse`);
          warehouseItems.quantity =
            parseInt(warehouseItems.quantity || 0) + parseInt(quantity);
          console.log(
            `Updated quantity for ${itemName}:`,
            warehouseItems.quantity
          );
        }
      }
