...
function getGSTLabel(debitNote) {
  const gstType = (debitNote.gstType || "").toString();
...
async function generateDebitNoteBuffer(debitNote, items = []) {
  ...
  const gstType = (debitNote.gstType || "").toUpperCase();
  ...
  const html = ejs.render(tpl, {
    ...
    companyAddress: debitNote.company?.address,
    companyGST: debitNote.company?.gstNumber,
    ...
    transport: debitNote.transport,
    vehicleNumber: debitNote.vehicleNumber,
    station: debitNote.station,
    gstType,
    gstRate: debitNote.gstRate,
    rows: preparedRows,
    pages,
    totalQty,
    subTotalCurrency,
