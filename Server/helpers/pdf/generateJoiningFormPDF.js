...
const generateJoiningFormPDF = async (req, res) => {
    try {
        const data = req.body; // Get dynamic data from request
        ...
        drawCell(startX + 150, startY + rowHeight * 6, data.dateOfBirth || "", 380, rowHeight);
        ...
        drawCell(startX + 150, startY + rowHeight * 7, data.interviewDate || "", 380, rowHeight);

        drawCell(startX, startY + rowHeight * 8, "Date of Joining", 150, rowHeight);
        drawCell(startX + 150, startY + rowHeight * 8, data.joiningDate || "", 380, rowHeight);
