/*
  Warnings:

  - You are about to drop the column `debitNoteId` on the `purchaseorderbill` table. All the data in the column will be lost.

*/
-- DropForeignKey
-- ALTER TABLE `purchaseorderbill` DROP FOREIGN KEY `PurchaseOrderBill_debitNoteId_fkey`;

-- DropIndex
-- DROP INDEX `PurchaseOrderBill_debitNoteId_fkey` ON `purchaseorderbill`;

-- AlterTable
ALTER TABLE `PurchaseOrder` ADD COLUMN `heading` VARCHAR(191) NULL,
    ADD COLUMN `termsConditionsId` VARCHAR(191) NULL;

-- AlterTable
-- ALTER TABLE `purchaseorderbill` DROP COLUMN `debitNoteId`;

-- AddForeignKey
ALTER TABLE `PurchaseOrder` ADD CONSTRAINT `PurchaseOrder_termsConditionsId_fkey` FOREIGN KEY (`termsConditionsId`) REFERENCES `Terms_Conditions`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
